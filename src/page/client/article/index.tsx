...
const Index: React.FC = () => {
  ...
  const { value: recommendList = [] } = useAsync(getSimilarArticles, { params: { id } });
  const { value: authorInfo, execute: getUserCardData, loading: userLoading } = useAsync(getUserCard, { immediate: false });
  ...
  const renderTags = (tags: TagType[]) => tags.map(
    ({ name, id }) => (<Tag
      key={name}
  ...
  useTimeout(() => countArticle({ id }), 12 * 1000);
  return (
          ...
          >
            <div className={styles.article}>
              <div className={styles.header}>
                <div className={mergeClassName(styles.title, 'text-ellipsis')}>{title}</div>
                ...
                {backgroundUrl && <img src={backgroundUrl} className={styles.cover} />}
          ...
          >
            {authorId && (
              <Card className={styles.user}>
                ...
                {
                  (github || email) && (
                    <div className={mergeClassName(styles.icons, styles.set_margin_to_children)}>
                      <a href={github} target="_blank"><GithubOutlined /></a>
