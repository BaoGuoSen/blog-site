...
const Index: FC = () => {
  ...
  const renderTags = (data: typeof tags) => {
    ...
    const maxViewCount = Math.max(...data.map(({ viewCount }) => viewCount))
    ...
    return [
      ...
      ...data
    ].map(({ id, name, viewCount, articleCount, icon }) => {
      const size = (() => {
      ...
      return (
        <Box3D
          ...
          text={{
            forward: {
              children: (
                <Badge count={articleCount} overflowCount={999}>
                  <span style={{ fontSize: Math.max(rate * 52, 20) }}>
                    {name}
                  </span>
                </Badge>
              )
            ...
            up: {
              children: icon && <img src={icon} className={styles.icon} />
            }
          }}
  ...
  const handleSearch = (searchVal?: string) => {
    ...
    const newTags = tags.filter(({ name }) =>
      new RegExp(searchVal, 'i').test(name)
    )
  ...
  return (
    <div className={styles.wrapper}>
      ...
      />
      <div className={styles.tags}>{renderTags(localTags)}</div>
    </div>
  )
}
